...
SessionList::SessionList(QDialog *parent)
    : QDialog(parent), ui(new Ui::SessionList),
...
SessionList::~SessionList() { delete ui; }

void SessionList::startSession() {
  QDBusInterface yakuake("org.kde.yakuake", "/yakuake/sessions",
                         "org.kde.yakuake");
  QDBusInterface yakuakeTabs("org.kde.yakuake", "/yakuake/tabs",
                             "org.kde.yakuake");
  QDBusInterface yakuakeWin("org.kde.yakuake", "/yakuake/window",
                            "org.kde.yakuake");
  ...
  if (yakuake.isValid()) {
    for (const QModelIndex &index : selectionModel->selectedIndexes()) {
      Session *session =
          (Session *)model.data(index, Qt::UserRole).value<void *>();
      QVariant sessionId =
          yakuake.call(SessionList::commandsMap[session->layout])
              .arguments()
              .takeFirst();
      QStringList terminalIds =
          yakuake.call("terminalIdsForSessionId", sessionId)
              .arguments()
              .takeFirst()
              .toString()
              .split(",");
      ...
      if (session->common_command.size()) {
        for (const QString &terminalId : terminalIds) {
          yakuake.call("runCommandInTerminal", terminalId.toInt(),
                       session->common_command);
        }
      }
      ...
      while (terminals.hasNext() && commands.hasNext()) {
        yakuake.call("runCommandInTerminal", terminals.next().toInt(),
                     commands.next());
      }
    ...
    QMessageBox::warning(this, "Warning",
                         "It seems that Yakuake is not running.");
  }
}
...
void SessionList::editSession() {
  Session *session =
      (Session *)model.data(ui->listView->currentIndex(), Qt::UserRole)
          .value<void *>();
...
void SessionList::selectionChanged() {
  bool isSingle =
      ui->listView->selectionModel()->selectedIndexes().length() == 1;
...
void SessionList::keyPressEvent(QKeyEvent *event) {
  ...
  if (combination.keyboardModifiers() & Qt::AltModifier) {
    ...
    if (combination.key() >= Qt::Key::Key_1 &&
        combination.key() <= Qt::Key::Key_9) {
...
void SessionList::keyReleaseEvent(QKeyEvent *event) {
  ...
  if (model.showHotkeys() &&
      !(event->keyCombination().keyboardModifiers() & Qt::AltModifier)) {
    model.setShowHotkeys(false);
  }
}

const char *SessionList::commandsMap[] = {
    "addSession", "addSessionTwoHorizontal", "addSessionTwoVertical",
    "addSessionQuad"};
