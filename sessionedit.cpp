...
SessionEdit::SessionEdit(QWidget *parent, Session *session, bool creationDialog)
  ...
  if (creationDialog) {
    ...
    for (int i = 0; i < 4; i++) {
      QLineEdit *editor =
          findChild<QLineEdit *>(QString("command%1Edit").arg(i));
      editor->setText(session->commands.value(i));
    }
    ui->favoriteCheckBox->setCheckState(session->favorite
                                            ? Qt::CheckState::Checked
                                            : Qt::CheckState::Unchecked);
  }
}

SessionEdit::~SessionEdit() { delete ui; }
...
void SessionEdit::deleteSession() { done(2); }
...
void SessionEdit::accept() {
  ...
  session->favorite =
      ui->favoriteCheckBox->checkState() == Qt::CheckState::Checked;
